...
namespace {
    Type type_from_str(std::string_view str) {
        if (str == "int") {
            return IntType{};
        } else if (str == "bool") {
            return BoolType{};
...
namespace std {
    ...
    struct std::formatter<Location> {
        ...
        auto format(const Location& location, std::format_context& ctx) const {
            return location.match(
                ...
                [&](const Loc& loc) {
                    return std::format_to(ctx.out(), "Loc {{ start: {}, end: {} }}", loc.start, loc.end);
                },
                [&](const LocWithFile& locWithFile) {
                    return std::format_to(ctx.out(), "LocWithFile {{ start: {}, end: {}, file: {} }}", locWithFile.loc.start, locWithFile.loc.end, locWithFile.file);
                }
            );
        }
    };
    ...
    struct std::formatter<Type> {
        ...
        auto format(const Type& type, std::format_context& ctx) const {
            return type.match(
                ...
                [&](const BoolType&) { return std::format_to(ctx.out(), "BoolType"); }
            );
        }
    };
    ...
    struct std::formatter<Value> {
        ...
        auto format(const Value& value, std::format_context& ctx) const {
            return value.match(
                [&](int64_t v) { return std::format_to(ctx.out(), "Value<int64_t>({})", v); },
                [&](bool v) { return std::format_to(ctx.out(), "Value<bool>({})", v); }
            );
        }
    };
        ...
        auto format(const Instruction& instr, std::format_context& ctx) const {
            return instr.match(
                [&](const ConstantInstr& c) {
                    return std::format_to(ctx.out(), "ConstantInstr {{ dest: {}, type: {}, value: {} }}", c.dest, c.type, c.value);
                },
                [&](const ValueInstr& v) {
                    return std::format_to(ctx.out(), "ValueInstr {{ op: {}, dest: {}, type: {}, args: {}, funcs: {}, labels: {} }}", v.op, v.dest, v.type, v.args, v.funcs, v.labels);
                },
                [&](const EffectInstr& e) {
                    return std::format_to(ctx.out(), "EffectInstr {{ op: {}, args: {}, funcs: {}, labels: {} }}", e.op, e.args, e.funcs, e.labels);
                }
            );
        }
    };
    ...
    template<>
    struct std::formatter<Argument> {
        constexpr auto parse(std::format_parse_context& ctx) {
            return ctx.begin();
        }

        auto format(const Argument& arg, std::format_context& ctx) const {
            return std::format_to(ctx.out(), "Argument {{ name: {}, type: {} }}", arg.name, arg.type);
        }
    };
    ...
    template<>
    struct std::formatter<Function> {
        constexpr auto parse(std::format_parse_context& ctx) {
            return ctx.begin();
        }

        auto format(const Function& func, std::format_context& ctx) const {
            return std::format_to(ctx.out(), "Function {{ name: {}, arguments: {}, type: {}, instrs: {} }}", 
...
Program::Program(json j) {
    for (const auto& func : j["functions"]) {
        Function function {
            .name = func["name"],
            .arguments = func.value("args", std::vector<std::string>{}) 
                | std::views::transform([](const json& arg) {
                    return Argument {
                        .name = arg["name"],
                        .type = type_from_str(arg["type"].get_ref<const std::string&>())
                    };
                }) | std::ranges::to<std::vector<Argument>>(),
            .type = type_from_str(func.value("type", "")),
        ...
        for (const auto& instr : func["instrs"]) {
            ...
            auto type = type_from_str(instr.value("type", ""));
            
            auto newInstr = [&]() -> Instruction {
                if (opName == "const") {
                    return ConstantInstr {
                      .dest = dest,
                      .type = type,
